...
/**
 * 168官方API数据源爬虫（使用主系统相同的API）
 */
class Source168Official extends BaseCrawler {
  constructor() {
    super('168Official', 'https://3650062.com/api');
  }
  ...
  async fetchHistoryData(lotCode, options = {}) {
    try {
      ...
      const endpoint = this.getEndpoint(lotCode);
      const historyEndpoint = this.getHistoryEndpointName(lotCode);
      let url = `${this.baseUrl}/${endpoint}/get${historyEndpoint}HistoryList.do?lotCode=${lotCode}&pageNo=${pageNo}&pageSize=${pageSize}`;
...
export default Source168Official;
